...
def rocks_fall_everyone_dies(tetris_board, highest_y_point, shape_cycle, gas_cycle):
    next_rock = next(shape_cycle)
    origin = (2, highest_y_point + 3)
    still_falling = True
    while still_falling:
        gas_symbol = next(gas_cycle)
        gas_dir = GAS_DIR_MAP[gas_symbol]  # gas blows
        for coord in next_rock:
            can_move = True
            try:
                delta_coord = tuple(numpy.add(coord, origin))
                if (tuple(numpy.add(delta_coord, gas_dir)) in tetris_board) or (
                        numpy.add(delta_coord, gas_dir)[0] < 0) or (tuple(numpy.add(delta_coord, gas_dir))[1] < 0) or (numpy.add(delta_coord, gas_dir)[0] > 6):
                    can_move = False
                    break
            except IndexError:
                can_move = False
        ...
        for coord in next_rock:
            ...
            if tuple(numpy.add(delta_coord, (0, -1))) in tetris_board or tuple(numpy.add(delta_coord, (0, -1)))[
                0] < 0 or tuple(numpy.add(delta_coord, (0, -1)))[1] < 0:
                still_falling = False
                break
        if not still_falling:
            for coord in next_rock:
                tetris_board.add(tuple(numpy.add(coord, origin)))
                highest_y_point = (max([coord[1] for coord in tetris_board]) + 1)
        else:
...
def part_one(input_filename):
    ...
    gas_cycle = cycle([char for char in input][0])
    highest_y_point = 0
    tetris_board = set()
    for _ in range(2022):
        new_peak = rocks_fall_everyone_dies(tetris_board, highest_y_point, shape_cycle, gas_cycle)
        highest_y_point = max(new_peak, highest_y_point)
    return highest_y_point
...
def part_two(input_filename):
    ...
    tetris_board = set()
    snapshots = {}
    for round in range(1000000):  # arbitrary "big enough" number
        new_peak = rocks_fall_everyone_dies(tetris_board, highest_y_point, shape_cycle, gas_cycle)
        highest_y_point = max(new_peak, highest_y_point)
        ...
        if snapshot in snapshots:
            ...
            cycle_length = round - round_entering_cycle
